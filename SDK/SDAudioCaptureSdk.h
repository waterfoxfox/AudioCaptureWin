...
//采集数据输出回调
typedef void (*OutputCapturedData)(unsigned char *data, int len, void *pObject);
...
* @return: 
*/
DLLIMPORT void  SDAudioCapture_Enviroment_Init(const char* outputPath, int outputLevel);

DLLIMPORT void  SDAudioCapture_Enviroment_Free();
...
*/
DLLIMPORT void*  SDAudioCapture_New();
...
* @return:
*/
DLLIMPORT void  SDAudioCapture_Delete(void** ppAudioCapture);
...
* @param nOutputByteNum: 采集单次输出的样点字节数
* @param eCapType: 采集的类型，麦克风或扬声器
* @param pfOutputCaptureData: 采集音频数据输出回调接口
* @param pObject: 上述输出回调接口的透传指针，将通过回调函数形参方式透传外层
* @return:
*/
DLLIMPORT BOOL  SDAudioCapture_Start(void* pAudioCapture, unsigned int nSampleRate, unsigned int nChannelNum, unsigned int nBitsPerSample, unsigned int nOutputByteNum, 
...
* @return:
*/
DLLIMPORT BOOL  SDAudioCapture_Stop(void* pAudioCapture);
